...
final allTransactionsViewModelProvider = ChangeNotifierProvider((ref)=> AllTransactionsViewModel(ref));
    ...
    try {
      ...
      if (response.success) {
        ...
        if (transactionData is List) {
          transactions = transactionData.map((item) => Transaction.fromJson(item as Map<String, dynamic>)).toList();
        } else {
